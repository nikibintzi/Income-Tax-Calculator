...
public class TaxpayerManager {
  ...
  private void updateFiles(int taxRegistrationNumber) throws IOException, WrongFileFormatException {
    if (new File(taxRegistrationNumber + "_INFO.xml").exists()) {
      System.out.println("__IN TAXPAYERMANAGER_____________IN UpdateFILES : XMLINFOWRITER____________");
      new XMLInfoWriter(this).generateFile(taxRegistrationNumber,"xml");
    } else {
      System.out.println("___IN TAXPAYERMANAGER____________IN UpdateFILES : TXTINFOWRITER____________");
      new TXTInfoWriter(this).generateFile(taxRegistrationNumber,"txt");
      return;
    }
    if (new File(taxRegistrationNumber + "_INFO.txt").exists()) {
      System.out.println("____IN TAXPAYERMANAGER___________IN UpdateFILES : TXTINFOWRITER____________");
      new TXTInfoWriter(this).generateFile(taxRegistrationNumber,"txt");
    }
  }
  ...
  public void saveLogFile(int taxRegistrationNumber, String fileFormat)
    ...
    if (fileFormat.equals("txt")) {
      TXTLogWriter writer = new TXTLogWriter(this);
      writer.generateFile(taxRegistrationNumber,fileFormat);
    } else if (fileFormat.equals("xml")) {
      XMLLogWriter writer = new XMLLogWriter(this);
      writer.generateFile(taxRegistrationNumber,fileFormat);
    } else {
